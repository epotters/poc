...
public class EvernoteClient {
  ...
  public EvernoteClient(String token) throws Exception {
    ...
    boolean versionOk = userStore
        .checkVersion("EvernoteClient", com.evernote.edam.userstore.Constants.EDAM_VERSION_MAJOR,
  ...
  public static void main(String args[]) throws Exception {
    ...
    EvernoteClient client = new EvernoteClient(token);

    try {

      String notebookName = "Blog";
      Notebook notebook = client.getNotebookByName(notebookName);
      client.listNotesInNotebook(notebook);

      // client.listNotes();
      // client.createNote();
      // client.searchNotes();
      // client.updateNoteTag();
  ...
  private static Data readFileAsData(String fileName) throws Exception {
    String filePath =
        new File(EvernoteClient.class.getResource(EvernoteClient.class.getCanonicalName() + ".class").getPath()).getParent()
  ...
  private Notebook getNotebookByName(String name) throws Exception {
    ...
    for (Notebook notebook : notebooks) {
      if (name.equals(notebook.getName())) {
        System.out.println("Notebook \"" + notebook.getName() + "\" found");
        return notebook;
      }
    }
    return null;
  ...
  private void createNote() throws Exception {
    // To create a new note, simply create a new Note object and fill in attributes such as the note's title.
    Note note = new Note();
    note.setTitle("Test note from poc.EvernoteClient.java");

    String fileName = "evernote-logo.png";
    String mimeType = "image/png";

    // To include an attachment such as an image in a note, first create a Resource for the attachment. At a minimum, the
    // Resource contains the binary attachment data, an MD5 hash of the binary data, and the attachment MIME type.
    ...
    // The content of an Evernote note is represented using Evernote Markup Language (ENML). The full ENML specification can
    // be found in the Evernote API Overview at http://dev.evernote.com/documentation/cloud/chapters/ENML.php
    ...
    // The new Note object that is returned will contain server-generated attributes such as the new note's unique GUID.
  ...
  private void searchNotes() throws Exception {
    ...
    // Learn more at http://dev.evernote.com/documentation/cloud/chapters/Searching_notes.php

    // In this example, we search for notes that have the term "hello" in the title.
    // This should return the sample note that we created in this demo app.
    String query = "intitle:hello";

    // To search for notes with a specific tag, we could do something like this: String query = "tag:tagname";
    // To search for all notes with the word "elephant" anywhere in them: String query = "elephant";
    ...
    while (iter.hasNext()) {
      ...
      // Note objects returned by findNotes() only contain note attributes such as title, GUID, creation date, update date,
      // etc. The note content and binary resource data are omitted, although resource metadata is included.
      // To get the note content and/or binary test.resources, call getNote() using the note's GUID.
  ...
  private void updateNoteTag() throws Exception {
    ...
    // Now update the note. Because we haven't set the content or test.resources, they won't be changed.
    noteStore.updateNote(note);
    System.out.println("Successfully added tag to existing note");

    // To prove that we didn't destroy the note, let's fetch it again and verify that it still has 1 resource.
